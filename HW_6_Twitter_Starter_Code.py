...
def construct_unique_key(baseurl, params):
    ...
    param_list.sort()
    unique_key = baseurl + underscore + underscore.join(param_list)
    return unique_key
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    # TODO: Implement function 
    hashtag_count = []
    cooccouring_hashtags = []

    for tweet in tweet_data['statuses']:
        hashtag_list = tweet['entities']['hashtags']
        for hashtag in hashtag_list:
            if(hashtag['text'].lower() != hashtag_to_ignore[1:].lower()):
                hashtag_count.append(hashtag['text'].lower())
    
    temp = collections.Counter(hashtag_count)
    max_value = -1
